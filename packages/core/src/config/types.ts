...
export type PopupConfigDirectives = Record<string, PopupDirective>
...
export interface IConfig {
	 ...
	 * 自定义指令集合
	 */
	directives: PopupConfigDirectives
	/**
	 * 开启调试模式
	 */
	debugMode: boolean
...
export type PopupConfigOption = {
	 ...
	 * 自定义弹出层指令
	 *
	 * - 你可以使用 `createPopupDirective()` 创建自定义指令，并且将
	 *   指令通过该属性注册。
	 * - 绑定的键名将用于生成指令的名称，例如键名 `test` 将注册为 `v-popup-test`
	 *   指令。
	 *
	 * @since 1.7.0
	 */
