...
## 1.7.0-beta.2 <Badge text="2026-09-20" />

### 特性

- 新增部分内部 `TS` 类型的导出，以供插件开发者使用。
...
::: tip Viewport - 视区
该版本中，我们引入了 `Viewport - 视区` 概念。

默认情况下，弹出层将以浏览器窗口为视区（和之前版本**保持一致**）。
...
::: tip Anchor - 锚点
该版本中，我们引入了 `Anchor - 锚点` 概念。

默认情况下，当未配置锚点元素时，其所有定位都将以视区为基础进行计算（和之前版本**保持一致**）。

当使用锚点元素时，弹出层的定位将基于锚点元素的四周进行计算，并且会跟随锚点元素的滚动行为实时更新其位置。

这一概念使得通过锚点元素定位弹出层成为可能，极大地提高了弹出层定位的灵活性。
:::

### 特性

- `render()` 函数新增 `viewport` 参数，用于设置弹出层的视区元素，默认值为 `null` ，此时使用浏览器窗口作为视区。
- `render()` 函数新增 `anchor` 参数，用于设置弹出层的锚点元素，默认值为 `null` ，即不使用锚点元素。
- `render()` 函数新增 `anchorPlacement` 参数，用于设置锚点弹出层的位置，默认值为 `top` ，即顶部居中对齐。
- `render()` 函数新增 `anchorFlip` 参数，用于设置锚点弹出层是否在视区空间不足时自动翻转，默认值为 `false`，即不翻转。
- `render()` 函数新增 `anchorFlipAdvance` 参数，用于设置锚点弹出层在视区空间不足时触发自动翻转，的提前偏移量，默认值为 `0`。
- `render()` 函数新增 `anchorShift` 参数，用于设置锚点弹出层是否在视窗空间不足时进行平移，默认值为 `false` ，即不进行平移。
- 新增 `PopupAnchorTrigger` 组件，用于通过组件的方式快速创建锚点弹出层，其内置了多种事件触发器，可通过自由组合实现各种复杂的弹出层触发交互逻辑。
...
- 修改 `popup.render()` 函数的 `disableScroll` 选项的默认值，现在为 `false` ，在此之前为 `true`。
...
- `createPopupPlus()` 函数的 `zIndex` 选项现在支持传入一个工厂函数，每次渲染弹出层时，会调用该函数获取一个新的 `z-index` 值。

## 1.6.1 <Badge text="2026-01-16" />

### 优化

- 优化底层渲染逻辑，从而为更多非组件环境提供渲染支持，提升兼容性。
- 优化性能，当使用 `PopupRoot` 根组件时，所有获取控制器的方法都将返回缓存中共享的无状态控制器实例，极大减少内存消耗。
...
- `render()` 函数移除 `maskClickClose` 参数，可使用功能更强大的 `maskDestroy` 参数代替。
