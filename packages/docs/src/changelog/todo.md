...
- <DVersionTodo version="1.7.0" level="high" author="STYZY" finish /> 新增 `viewport` 参数，用于设置弹出层的视口元素选择器，默认值为 `undefined` 。
- <DVersionTodo version="1.7.0" level="high" author="STYZY" finish /> 新增 `anchor` 参数，用于设置弹出层的锚点元素，默认值为 `undefined` 。
- <DVersionTodo version="1.7.0" level="high" author="STYZY" finish /> 新增 `anchorPlacement` 参数，用于设置锚点弹出层的位置，默认值为 `top` 。
- <DVersionTodo version="1.7.0" level="high" author="STYZY" finish /> 新增 `anchorFlip` 参数，用于设置锚点弹出层是否在空间不足时自动翻转，默认值为 `false` 。
- <DVersionTodo version="1.7.0" level="high" author="STYZY" finish /> 新增 `anchorFlipAdvance` 参数，用于设置锚点弹出层在空间不足时自动翻转的偏移量，默认值为 `0` 。
- <DVersionTodo version="1.7.0" level="high" author="STYZY" finish /> 新增 `anchorShift` 参数，用于设置锚点弹出层是否在空间不足时的自动平移方式，默认值为 `none` 。
...
- <DVersionTodo version="1.6.1" level="high" author="STYZY" finish /> 修改 `zIndex` 参数，支持传入一个工厂函数，返回一个 `number` 类型的 `z-index` 值。
...
- <DVersionTodo version="1.6.0" level="high" author="STYZY" finish /> `Dialog 对话` 新增 `maskTransparent` 参数，用于设置是否启用透明遮罩层，默认值为 `false` 。
