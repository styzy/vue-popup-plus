import { type Component } from 'vue'
import {
	definePlugin,
	LogType,
	LogGroupItemType,
	printLog,
	type AnchorPlacement,
	type IController,
	type InstanceId,
	type MaskDestroyHandler,
	type ExtractComponentPropTypes,
} from 'vue-popup-plus'
import { PluginLog } from '../../log'
import type {
	GlobalPluginConfig,
	MergedOption,
	SharedOption,
} from '../../typings'
import { requiredCoreVersion } from '../../version'

class Log extends PluginLog {
	namespace = 'VuePopupPlusPluginPreset Alert'
}

type BubbleOption<TComponent extends Component = Component> = {
	/**
	 * 气泡锚点元素
	 *
	 * - 可配合 `placement` 参数指定弹出层相对于锚点的对齐方式
	 * - 传入字符串时，会根据字符串选择器查询元素
	 */
	el: HTMLElement | string
	/**
	 * 气泡内容组件
	 */
	component: TComponent
	/**
	 * 气泡内容组件props
	 */
	componentProps?: ExtractComponentPropTypes<TComponent>
	/**
	 * 对话框渲染完成时调用的回调函数
	 */
	onMounted?: () => void
	/**
	 * 对话框宽度
	 *
	 * - 默认值为 `auto`
	 * - 支持 `string` 或 `number` 类型
	 */
	width?: string | number
	/**
	 * 对话框最大宽度
	 *
	 * - 默认值为 `100%`
	 * - 支持 `string` 或 `number` 类型
	 */
	maxWidth?: string | number
	/**
	 * 对话框最小宽度
	 *
	 * - 默认值为 `auto`
	 * - 支持 `string` 或 `number` 类型
	 */
	minWidth?: string | number
	/**
	 * 对话框高度
	 *
	 * - 默认值为 `auto`
	 * - 支持 `string` 或 `number` 类型
	 */
	height?: string | number
	/**
	 * 对话框最大高度
	 *
	 * - 默认值为 `100%`
	 * - 支持 `string` 或 `number` 类型
	 */
	maxHeight?: string | number
	/**
	 * 对话框最小高度
	 *
	 * - 默认值为 `auto`
	 * - 支持 `string` 或 `number` 类型
	 */
	minHeight?: string | number
	/**
	 * 气泡相对于锚点的对齐方式
	 *
	 * - 默认值：`top`
	 */
	placement?: AnchorPlacement
	/**
	 * 是否显示箭头
	 *
	 * - 默认值为 `true`
	 */
	arrow?: boolean
	/**
	 * 是否显示遮罩层
	 *
	 * - 默认值为 `false`
	 */
	mask?: boolean
	/**
	 * 遮罩层是否模糊
	 *
	 * - 默认值：`false`
	 * - 仅在 `mask` 参数为 `true` 时有效
	 */
	maskBlur?: boolean
	/**
	 * 遮罩层是否透明
	 *
	 * - 默认为 `false`
	 * - 优先级高于 `maskBlur`
	 * - 仅在 `mask` 参数为 `true` 时有效
	 */
	maskTransparent?: boolean
	/**
	 * 点击遮罩层是否关闭气泡
	 *
	 * - 默认值为 `false`
	 * - 仅在 `mask` 参数为 `true` 时有效
	 * - 传入 `true` ，点击遮罩层将关闭气泡
	 * - 可传入一个函数，该函数接收一个 `(payload?: any) => Promise<void>`
	 *   类型的函数作为参数，执行后将关闭气泡，可传入关闭携带的负载参数，返回的
	 *   `Promise` 对象会在气泡关闭动画完成后 `resolve()` 。
	 * - 仅在 `mask` 参数为 `true` 时有效
	 *
	 * - 使用示例：
	 * ```ts
	 * popup.bubble({
	 *     el: '#anchor',
	 *     component: () => import('./HelloWorld.vue'),
	 *     maskClose: async (close)=>{
	 *         if(...自定义拦截条件) return
	 *
	 *         // 直接关闭
	 *         close('携带的关闭参数')
	 *
	 *         // 异步等待关闭动画结束
	 *         await close('携带的关闭参数')
	 *         // 关闭后执行其他操作
	 *     },
	 * })
	 * ```
	 */
	maskClose?: boolean | MaskDestroyHandler
} & SharedOption

export interface IBubble {
	/**
	 * 显示气泡
	 *
	 * - 如果需要等待气泡关闭后再执行后续代码，需要通过 `await`
	 *   调用，等待执行结束后继续执行后续代码
	 * - 使用示例：
	 *
	 * ```ts
	 * popup.bubble({
	 *     el: '#anchor',
	 *     component: () => import('./HelloWorld.vue'),
	 * })
	 * // 即使气泡不关闭，也不会阻塞后续代码执行
	 *
	 * await popup.bubble({
	 *     el: '#anchor',
	 *     component: () => import('./HelloWorld.vue'),
	 * })
	 * // 只有气泡关闭后，才会继续执行后续代码
	 * ```
	 */
	<TComponent extends Component = Component>(
		this: IController,
		options: BubbleOption<TComponent>
	): Promise<void>
}

export interface IBubbleClose {
	/**
	 * 关闭气泡
	 *
	 * - 将会关闭最后一个创建的气泡
	 * - 如果当前没有气泡正在显示，则不会有任何效果，调试模式下会抛出警告
	 * - 可传递任意类型的参数，该参数将会被传递给打开气泡时的 Promise resolve 函数
	 * @param payload 关闭时传递的参数
	 */
	<T extends any = any>(this: IController, payload?: T): Promise<void>
}

type BubbleDefaultOption = Omit<
	BubbleOption,
	'component' | 'componentProps' | 'el' | 'zIndex'
>

export type BubbleConfig = GlobalPluginConfig & {
	/**
	 * 默认选项
	 *
	 * - 统一配置 `popup.bubble()` 方法的默认选项
	 */
	defaultOptions?: BubbleDefaultOption
}

let seed = 1

const createId = () => `bubble-${seed++}`

export const bubble = definePlugin({
	name: 'plugin-preset-bubble',
	author: 'STYZY',
	requiredCoreVersion,
	install: (
		config,
		{ skin = 'modern', defaultOptions = {} }: BubbleConfig = {}
	) => {
		const recordList: Array<{
			id: string
			instanceId: InstanceId
			resolve: (payload?: any) => void
		}> = []

		const bubble: IBubble = function ({
			el,
			component,
			componentProps = {},
			onMounted = () => {},
			width = defaultOptions.width ?? 'auto',
			maxWidth = defaultOptions.maxWidth ?? '100%',
			minWidth = defaultOptions.minWidth ?? 'auto',
			height = defaultOptions.height ?? 'auto',
			maxHeight = defaultOptions.maxHeight ?? '100%',
			minHeight = defaultOptions.minHeight ?? 'auto',
			placement = defaultOptions.placement ?? 'top',
			arrow = defaultOptions.arrow ?? true,
			mask = defaultOptions.mask ?? false,
			maskBlur = defaultOptions.maskBlur ?? false,
			maskTransparent = defaultOptions.maskTransparent ?? false,
			maskClose = defaultOptions.maskClose ?? false,
			zIndex,
		}) {
			return new Promise<void>((resolve) => {
				const id = createId()
				const instanceId = this.render({
					component: () => import('./src/PBubble.vue'),
					componentProps: {
						skin,
						id,
						customComponent: component,
						customComponentProps: componentProps,
						placement,
						arrow,
						debugMode: config.debugMode,
						onClose: () => {
							this.destroy(instanceId)
						},
					},
					anchor: el,
					anchorPlacement: placement,
					width,
					maxWidth,
					minWidth,
					height,
					maxHeight,
					minHeight,
					mask,
					maskBlur,
					maskTransparent,
					maskDestroy: maskClose,
					disableScroll: mask,
					zIndex,
					onMounted: () => {
						const mergedOptions: MergedOption<BubbleOption> = {
							el,
							component,
							componentProps,
							onMounted,
							width,
							maxWidth,
							minWidth,
							height,
							maxHeight,
							minHeight,
							placement,
							arrow,
							mask,
							maskClose,
							maskBlur,
							maskTransparent,
							zIndex,
						}

						printLog(
							new Log({
								type: LogType.Info,
								caller: {
									name: 'popup.bubble()',
									type: 'function',
									value: bubble,
								},
								message: `打开气泡 ${id} 成功`,
								group: [
									{
										type: LogGroupItemType.Data,
										title: '控制器',
										dataName: this.id,
										dataValue: this,
										dataType: 'IController',
									},
									{
										type: LogGroupItemType.Info,
										title: '气泡ID',
										content: id,
										important: true,
									},
									{
										type: LogGroupItemType.Data,
										title: '调用参数',
										dataName: 'options',
										dataValue: arguments[0],
										dataType: 'BubbleOption',
									},
									{
										type: LogGroupItemType.Data,
										title: '合并参数',
										dataName: 'mergedOptions',
										dataValue: mergedOptions,
										dataType: 'Required<BubbleOption>',
									},
								],
							})
						)

						onMounted()
					},
					onUnmounted: (payload?: any) => {
						const index = recordList.findIndex(
							(item) => item.id === id
						)

						if (index !== -1) {
							// id 存在，代表气泡并非被 bubbleClose 关闭
							recordList.splice(index, 1)

							resolve(payload)
						}
					},
				})

				recordList.push({
					id,
					instanceId,
					resolve,
				})
			})
		}

		const bubbleClose: IBubbleClose = async function (payload) {
			const { id, instanceId, resolve } = recordList.pop() || {}

			if (!instanceId) {
				printLog(
					new Log({
						type: LogType.Warning,
						caller: {
							name: 'popup.bubbleClose()',
							type: 'function',
							value: bubbleClose,
						},
						message: `关闭气泡失败，当前没有正在显示的气泡`,
						group: [
							{
								type: LogGroupItemType.Data,
								title: '控制器',
								dataName: this.id,
								dataValue: this,
								dataType: 'IController',
							},
							{
								type: LogGroupItemType.Data,
								title: '携带参数',
								dataName: 'payload',
								dataValue: payload,
								dataType: 'any',
							},
						],
					})
				)
				return
			}

			await this.destroy(instanceId, payload)

			printLog(
				new Log({
					type: LogType.Info,
					caller: {
						name: 'popup.dialogClose()',
						type: 'function',
						value: bubbleClose,
					},
					message: `关闭气泡 ${id} 成功`,
					group: [
						{
							type: LogGroupItemType.Data,
							title: '控制器',
							dataName: this.id,
							dataValue: this,
							dataType: 'IController',
						},
						{
							type: LogGroupItemType.Info,
							title: '对话框ID',
							content: id!,
							important: true,
						},
						{
							type: LogGroupItemType.Data,
							title: '携带参数',
							dataName: 'payload',
							dataValue: payload,
							dataType: 'any',
						},
					],
				})
			)

			resolve!(payload)
		}

		config.customProperties.bubble = bubble
		config.customProperties.bubbleClose = bubbleClose
	},
})

declare module 'vue-popup-plus' {
	interface PopupCustomProperties {
		bubble: IBubble
		bubbleClose: IBubbleClose
	}
}
